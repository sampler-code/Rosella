...
// القائمة
// البيانات الأساسية (عناوين السكشنات والفرعية)
 const sectionsData = {
  "Products": [
    "All Products",
    "On Sale"
...
// إضافة العناصر للقائمة في الـ sidebar
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!searchOverlay || !closeSearchBtn || !searchSubmitBtn || !searchIcon || !searchInput) {
    ...
    return; // يوقف تنفيذ الكود إذا العناصر مش موجودة
  }
 ...
 function renderOnSale() {
  ...
  // جمع كل العناصر في مصفوفة واحدة مع info القسم
  const allItems = [];
  ...
  products.forEach(product => {
    if (product.onSale) {
      ...
      allItems.push({
        ...
        image: variant.image,
        price: variant.price,
        discount: product.discount || 0.20,  // خصم افتراضي 20% لو مش محدد
        section: "Product"
      });
    }
  });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", function (e) {
    ...
    // أضف هذا هنا فورًا لإظهار الأخطاء عند وجودها
    form.classList.add("was-validated");
    ...
    if (!form.checkValidity()) {
      // حقل أو أكثر غير معبّى صح => لا ترسل
      return;
    }

    // لو وصلنا لهنا، يعني كل الحقول صحيحة
