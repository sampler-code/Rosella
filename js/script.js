...
/* فورم الفيدباك*/ 
 // تعريف النجوم والتقييم مرة وحدة
const stars = document.querySelectorAll("#starRating i");
const ratingInput = document.getElementById("rating");

//  تفعيل تأثير النجوم (Hover + Select)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // استدعاء المستخدمين من localStorage أو تهيئة مصفوفات جديدة
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (userIcon) {
    userIcon.addEventListener("click", function(e) {
      e.preventDefault();
      // لو أنت حالياً بصفحة products.html (مثلاً)، رجع ل index.html مع هاشتاغ #login
      if (window.location.pathname.includes("products.html")) {
        window.location.href = "index.html#login";
      } else {
        // لو أنت في نفس الصفحة فيها سكشن login، نعمل scroll smooth
...
sidebarToggle.addEventListener("click", () => {
  ...
  sidebarToggle.style.display = "none"; // تخفي الايقونة
});

closeBtn.addEventListener("click", () => {
  sidebar.style.left = "-300px";
  sidebarToggle.style.display = "block"; // تظهر الايقونة
});

// يغلق عند الضغط على رابط
